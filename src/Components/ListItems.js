      ...
      {items?.map((item) => (
        <div className="bg-blue-200 bg-opacity-50 px-10 flex justify-between  py-2 border-b-4 border-indigo-500 border-opacity-5" key={item?.card?.info?.id} >
          ...
          <div className="w-3/12 h-auto flex justify-evenly relative">
            <img className="  h-10" src={"https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/" + item?.card?.info?.imageId} alt="listImage"/>
            <div className=" absolute bg-primary rounded px-4 py-1 text-xs shadow-lg text-green-600 bottom-0">Add+</div>
          </div>
          </div>
      ))}
    </div>
